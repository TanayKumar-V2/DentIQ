...
function VapiWidget() {
  ...
  useEffect(() => {
    const parseError = (rawError: unknown) => {
      const e = rawError as {
        type?: string;
        stage?: string;
        error?: { message?: string; errorMsg?: string; errorDetail?: string };
        message?: string;
        reason?: string;
      };
      const deepMessage =
      ...
      if (typeof deepMessage === "string") return deepMessage;
      if (e?.type) return `Voice call failed (${e.type}${e.stage ? ` at ${e.stage}` : ""}).`;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 flex flex-col overflow-hidden pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <Card className="bg-card/90 backdrop-blur-sm border border-border overflow-hidden relative">
          <div className="aspect-video flex flex-col items-center justify-center p-6 relative">
            ...
            <h2 className="text-xl font-bold text-foreground">DentIQ AI</h2>
          ...
          <div className="space-y-3">
            {messages.map((msg, index) => (
              <div key={index} className="message-item animate-in fade-in duration-300">
                <div className="font-semibold text-xs text-muted-foreground mb-1">
                  {msg.role === "assistant" ? "DentIQ AI" : "You"}:
                </div>
            ...
            {callEnded && (
              <div className="message-item animate-in fade-in duration-300">
                ...
                <p className="text-foreground">Call ended. Thank you for using DentIQ AI!</p>
              </div>
            )}
          </div>
        </div>
      ...
      {callError && (
        <div className="w-full mb-6 rounded-lg border border-destructive/30 bg-destructive/10 p-3">
          <p className="text-sm text-destructive">{callError}</p>
          {callError.toLowerCase().includes("meeting has ended") && (
            <p className="text-xs text-destructive/80 mt-2">
              This usually means the assistant/key/workspace is mismatched or the Daily room was
              closed before join.
            </p>
          )}
        </div>
